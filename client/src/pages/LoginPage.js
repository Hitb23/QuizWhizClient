...
const LoginPage = () => {
  const [formValues, setFormValues] = useState({
    ...
    password: {
      ...
      invalidErrorMessage: "Password must have uppercase, lowercase, number, and special character",
    },
  });
  ...
  const validateField = (name, value) => {
    switch (name) {
      case "email":
        if (value.trim() === "") return { isValid: false, errorType: "required" };
        if (!validateEmail(value)) return { isValid: false, errorType: "invalid" };
        return { isValid: true, errorType: null };
      case "password":
        if (value.trim() === "") return { isValid: false, errorType: "required" };
        if (!validatePassword(value)) return { isValid: false, errorType: "invalid" };
        return { isValid: true, errorType: null };
      default:
        return { isValid: true, errorType: null };
    }
  ...
  const handleSubmit = (e) => {
    ...
    console.log("Email: " + formValues.password.value);
  };
